...
export default function ViewProperties(props){
        ...
        return images.map((item,index)=>{
            return(
                <div onMouseEnter={()=>handleShow(index)}>
                    <img src={`${serverURL}/images/${item}`} width='100%' height='100%' onClick={handleNavigate}  style={{borderRadius:12}}/>
                </div>
    ...
    const handleShow=(index)=>{
        if(index){
          setButton2Style({display: 'flex',position: "absolute",left: 5,top: 130,zIndex: 2,border: 0,borderRadius: 13,width: 26,height: 26,backgroundColor: "#fff",color: "#000",cursor: "pointer"})
...
return(
    <>
<div style={{display:'inline-list-item',position:'relative',width:270,height:270,margin:40,cursor:'pointer'}} 
onMouseEnter={e => {
        ...
        <div style={{fontSize:14,marginLeft:8,fontWeight:'bolder'}}>{data.title}</div>
